export default {
    ...
    execute: async (sock, chat, m) => {
        ...
        if (!horaTexto) return sock.sendMessage(chat, { text: `✳️ Usa: *${m.prefix||'.'}2vs2 [hora]*` })
        ...
        if(participantes.length<6) return sock.sendMessage(chat,{text:"⚠️ Necesito al menos 6 usuarios."})
        const t1=participantes.slice(0,2), t2=participantes.slice(2,4), sup=participantes.slice(4,6)
        const render = a=>a.map((u,i)=>`${i===0?"👑":"🥷🏻"} ┇ @${u.id.split("@")[0]}`).join("\n")
        const txt = `*2 𝐕𝐄𝐑𝐒𝐔𝐒 2*\n\n⏱ 𝐇𝐎𝐑𝐀𝐑𝐈𝐎\n${horaMsg}\n\n 𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 1\n${render(t1)}\n\n 𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 2\n${render(t2)}\n\n ʚ 𝐒𝐔𝐏𝐋𝐄𝐍𝐓𝐄𝐒:\n${render(sup)}`
