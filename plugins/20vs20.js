export default {
    comando: '20vs20', alias: ['vs20'], category: 'freefire', cooldown: 5, group: true,
    execute: async (sock, chat, m) => {
        const horaTexto = m.text.split(' ').slice(1).join(' ').trim()
        if (!horaTexto) return sock.sendMessage(chat, { text: `✳️ Usa: *${m.prefix||'.'}20vs20 [hora]*` })
        ...
        if(participantes.length<42) return sock.sendMessage(chat,{text:"⚠️ Necesito al menos 42 usuarios para un 20vs20."})
        const t1=participantes.slice(0,20), t2=participantes.slice(20,40), sup=participantes.slice(40,42)
        const render = a=>a.map((u,i)=>`${i===0?"👑":"🥷🏻"} ┇ @${u.id.split("@")[0]}`).join("\n")
        const txt = `*20 𝐕𝐄𝐑𝐒𝐔𝐒 20*\n\n⏱ 𝐇𝐎𝐑𝐀𝐑𝐈𝐎\n${horaMsg}\n\n 𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 1\n${render(t1)}\n\n 𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 2\n${render(t2)}\n\n ʚ 𝐒𝐔𝐏𝐋𝐄𝐍𝐓𝐄𝐒:\n${render(sup)}`
