export default {
    comando: '6vs6', alias: ['vs6'], category: 'freefire', cooldown: 5, group: true,
    execute: async (sock, chat, m) => {
        const horaTexto = m.text.split(' ').slice(1).join(' ').trim()
        if (!horaTexto) return sock.sendMessage(chat, { text: `✳️ Usa: *${m.prefix||'.'}6vs6 [hora]*` })
        ...
        if(participantes.length<14) return sock.sendMessage(chat,{text:"⚠️ Necesito al menos 14 usuarios."})
        const t1=participantes.slice(0,6), t2=participantes.slice(6,12), sup=participantes.slice(12,14)
        const render = a=>a.map((u,i)=>`${i===0?"👑":"🥷🏻"} ┇ @${u.id.split("@")[0]}`).join("\n")
        const txt = `*6 𝐕𝐄𝐑𝐒𝐔𝐒 6*\n\n⏱ 𝐇𝐎𝐑𝐀𝐑𝐈𝐎\n${horaMsg}\n\n 𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 1\n${render(t1)}\n\n 𝗘𝗦𝗖𝗨𝗔𝗗𝗥𝗔 2\n${render(t2)}\n\n ʚ 𝐒𝐔𝐏𝐋𝐄𝐍𝐓𝐄𝐒:\n${render(sup)}`
